...
def make_parser():
    ...
    parser.add_argument(
        "-cfg",
        "--config",
        default="/home/droneyee/video_analyst/experiments/mymodel/test/mobilenet_l4_test_fusion.yaml", #mobilenet_large_test_fusion
        type=str,
    ...
    parser.add_argument("-v",
                        "--video",
                        type=str,
                        default="/home/yinyuxiao/video_analyst/test.mp4",
    ...
    while vs.isOpened():
        ...
        if ret:
            if init_box is not None:
                ...
                fps = (fps + (1. / (time.time() - time_a))) / 2  # 此处的time.time()就是检测完这张图片的结束时间,除以2是为了和之前的fps求一个平均
                ...
                cv2.putText(show_frame,
                            "track cost: {:.4f} s,fps: {:.2f} fps".format(time_cost,fps), (128, 20),
