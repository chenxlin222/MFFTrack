...
class MobileNetV3SmallImpl(BaseModule):
    def __init__(self, cfgs_mode, width_mult=1., used_layers=[6, 8, 11],
                 feat_channel=[40, 48, 96], feat_fx_size=[8, 8, 4], feat_fz_size=[19, 19, 10],
                 final_channel=48, num_heads=3, num_layers=2, prob_dropout=0.0, init_cfg=None):
        ...
        for idx, layer in enumerate(self.used_layers):
            self.position_embeeding_fx_dict[str(layer)] = SpatialPositionEncodingLearned(final_channel,
                                                                                         self.feat_fx_size[idx])

        for idx, layer in enumerate(self.used_layers):
            self.position_embeeding_fz_dict[str(layer)] = SpatialPositionEncodingLearned(final_channel,
        ...
        self.encoder = Encoder(mid_channels_model=final_channel,
                               mid_channels_ffn=final_channel * 4,
                               num_heads=num_heads,
    ...
    def forward(self, x):
        output = OrderedDict()
        results = list()
        self.position_embeeding = self.position_embeeding_fz_dict if x.shape[
                                                                         -1] == 127 else self.position_embeeding_fx_dict
        self.shape = self.feat_fz_size if x.shape[-1] == 127 else self.feat_fx_size
        ...
        enc_output,cat_out = self.encoder(cat_input)
        output = torch.split(enc_output, [self.shape[i] ** 2 for i in range(len(self.shape))], dim=0)[-2]
        L, B, C = output.shape
...
class MobileNetV3_fusion(ModuleBase):
    ...
    def __init__(self):
        super(MobileNetV3_fusion, self).__init__()
