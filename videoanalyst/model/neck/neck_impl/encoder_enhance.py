...
class EncoderLayer_siamtpn(nn.Module):
    ''' Compose with two layers '''

    def __init__(self, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        super(EncoderLayer_siamtpn, self).__init__()
        self.slf_attn = nn.MultiheadAttention(embed_dim=d_model, num_heads=n_head, dropout=dropout)
        self.norm = nn.LayerNorm(d_model, eps=1e-6)
        self.pos_ffn = PositionWiseFeedForward(d_model, d_inner, dropout=dropout)

    def forward(self, enc_input,enc_kv, mask=None):
        enc_output, enc_slf_attn = self.slf_attn(query=enc_input, key=enc_kv, value=enc_kv,
                                                 attn_mask=mask)
        enc_output = enc_input + enc_output
        enc_output = self.norm(enc_output)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for enc_layer in self.encoder_layers:
            ...
            out_results.append(enc_output)
        cat_out = torch.cat(out_results, dim=2)
        # cat_out = 0
        return enc_output,cat_out  # HW, B, C

class Encoder_l42(nn.Module):
    def __init__(self,
                 mid_channels_model=256,
                 mid_channels_ffn=2048,
                 num_heads=8,
                 ...
                 prob_dropout=0.0,
                 score_size=33):
        super(Encoder_l42, self).__init__()
        assert mid_channels_model % num_heads == 0
        mid_channels_k = mid_channels_model // num_heads
        mid_channels_v = mid_channels_k
...
class Encoder_siamtpn(nn.Module):
    def __init__(self,
                 mid_channels_model=256,
                 mid_channels_ffn=2048,
                 num_heads=8,
                 ...
                 prob_dropout=0.0,
                 score_size=33):
        super(Encoder_siamtpn, self).__init__()
        assert mid_channels_model % num_heads == 0
        mid_channels_k = mid_channels_model // num_heads
        mid_channels_v = mid_channels_k
        ...
        # self.layer_norm = nn.LayerNorm(mid_channels_model, eps=1e-6)
        self.encoder_layers = nn.ModuleList([
            EncoderLayer_siamtpn(mid_channels_model, mid_channels_ffn, num_heads,
                         mid_channels_k, mid_channels_v, dropout=prob_dropout)
            for _ in range(num_layers)])
    ...
    def forward(self, x,enc_kv):
        ...
        # no need of mask if enc_output.shape is (BT, HW, C)
        for enc_layer in self.encoder_layers:
            enc_output, enc_slf_attn = enc_layer(enc_output,enc_kv)
            out_results.append(enc_output)
        cat_out = torch.cat(out_results, dim=2)
        # cat_out = 0
        return enc_output,cat_out  # HW, B, C
