...
class EFTracker(PipelineBase):
    r"""
    Basic EFTracker tracker
    ...
    def __init__(self, *args, **kwargs):
        super(EFTracker, self).__init__(*args, **kwargs)
        self.update_params()
...
class SiamFCppTracker(PipelineBase):
    ...
    def track(self,
        ...
        score = tensor_to_numpy(score[0])[:, 0]
        # ###########################################
        # score_1 = sc.view(19,19)
        # # xf1 = score_1.cpu().detach().numpy()
        # # xf2 = torch.zeros(19, 19).cpu().detach().numpy()
        # # for x in range(256):
        # #     xf2 = xf2 + xf1[0][x]
        # vutils.save_image(score_1, '/home/yinyuxiao/video_analyst/main/test')
        # plt.imshow(score_1, cmap='viridis', origin='upper')
        # plt.title('Window')
        # plt.colorbar()
        # # 保存图片
